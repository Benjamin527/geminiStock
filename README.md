...
这是一个针对 `CONL` / `TSLL` 的 AI 盯盘与报警原型。系统只做研究提醒，不做自动下单，不构成投资建议。
...
- 夜盘：20:00-04:00，默认每 5 分钟运行一次，主要用于纯价格异动预警。
- 盘前：04:00-09:30，默认每 5 分钟运行一次。
- 盘中：09:30 起前 60 分钟每 1 分钟运行一次，之后到 12:00 每 5 分钟运行一次，12:00 到 14:30 每 10 分钟运行一次，14:30 到收盘每 2 分钟运行一次。
- 盘后：16:00-20:00，每 60 分钟运行一次。
- 休市：不自动执行，等待下一个夜盘或盘前时段。

如果当前时间落在北京时间 `23:30` 到次日 `09:30`，系统会把过快的调度自动降到最多每 30 分钟一次，保留常驻运行但减少夜间和清晨资源占用。调度也会识别常见 NYSE 休市日和 13:00 提前收盘日；长等待会拆成最多 5 分钟的小睡眠，避免电脑睡眠 / Docker 恢复后长时间卡在旧倒计时里。
...
本地只读控制台用于查看系统状态、最近 AI 分析、是否传图、技术事件、报警记录和已生成的复核图。它包含移动端优先的主观察卡片和手机卡片流，不提供下单、强制报警或修改策略的入口。控制台默认只读取后台已经落库的最近 1 分钟 K 线和技术快照，不在页面定时刷新时额外请求实时行情源，避免前台刷新造成 yfinance / Polygon 限流。
...
报警会被分成 `info`、`watch`、`strong` 三档写入数据库 payload。强买入是 `strong`；普通 setup 是 `watch`；弱信号和 no-trade 相关提醒是 `info`。
...
系统另有独立的“价格异动预警”，覆盖主 watchlist 和 `MOVEMENT_ALERT_SYMBOLS` 中的纯异动标的，默认包含 `BTC-USD`。其中纯异动标的每 30 分钟扫描一次，并检查最近 30 分钟内的快速下跌或快速拉升，不依赖 AI 买入判断；默认三档阈值是 1.2%、2.0%、3.0%。达到 1 档推送 1 次，2 档推送 2 次，3 档推送 3 次；Dashboard 的“价格异动阈值”区域可以直接修改三档阈值，保存后后台下一轮扫描会读取新配置。BTC 只进入价格异动预警，不进入 AI 分析、主观察卡片或价格到位提醒。
