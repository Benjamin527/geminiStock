...
本地只读控制台用于查看系统状态、最近 AI 分析、是否传图、技术事件、报警记录和已生成的复核图。它包含移动端优先的主观察卡片、大盘路径预期和手机卡片流，不提供下单、强制报警或修改策略的入口。控制台默认只读取后台已经落库的最近 1 分钟 K 线和技术快照，不在页面定时刷新时额外请求实时行情源，避免前台刷新造成 yfinance / Polygon 限流。
